...
1)Подготовка Атакующим Запроса: Злоумышленник создает вредоносную веб-страницу или email с HTML-кодом, который автоматически
...
2)Выполнение Запроса: Когда пользователь, который аутентифицирован на `example.com`, открывает эту вредоносную страницу или email,
 форма автоматически отправляется. Поскольку запрос отправляется с аутентификационными данными пользователя
  (например, сессионными cookies), сервер `example.com` обрабатывает его как законный и изменяет email-адрес пользователя на адрес
  злоумышленника.
